...
def detect_landmarks(img):
    ...
    img_gray = img
    if len(img.shape) > 2 and img.shape[2] == 3:  # Check if image has 3 channels (RGB/BGR)
        img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    elif len(img.shape) > 2 and img.shape[2] == 4:  # Check if image has 4 channels (RGBA/BGRA)
